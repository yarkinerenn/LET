...
const COLORS = ['#FF4136', '#2ECC40', '#FFBB28', '#FF8042'];
...
function toHotelLabel(val: string | number | undefined): "Deceptive" | "Truthful" | undefined {
  if (val === undefined || val === null) return undefined;
  const t = String(val).trim().toUpperCase().replace(/[.\s]/g, '').replace(/[()]/g, '');
  if (t === 'DECEPTIVE' || t === 'A') return 'Deceptive';
  if (t === 'TRUTHFUL' || t === 'B') return 'Truthful';
  return undefined;
}
      ...
      {loading ? (
        ...
        <>
          ...
          <Row className="mb-4 align-items-center justify-content-between">
            <Col md="auto">
              <h2 className="mb-2">Hotel Classification Report</h2>
            ...
            ← Back to datasetview
              ...
              {dataType !== 'hotel' && (
                <Alert variant="warning" className="mt-3 mb-0 p-2">
                  This run is <strong>{dataType}</strong>, but you opened the Hotel page. Metrics/UI here expect Deceptive/Truthful.
                </Alert>
              )}
            </Col>
            <Col md="auto">
          ...
          <Row className="mb-4">
            <Col md={6}>
              <Card className="h-100">
                <Card.Body>
                  <Card.Title>Hotel Label Distribution</Card.Title>
                      ...
                      {paginatedResults?.map((result, index) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td>
                              <Badge bg={pred === 'Deceptive' ? 'danger' : 'success'}>{pred || String(result.label)}</Badge>
                            </td>
                            <td>{(result.score * 100).toFixed(1)}%</td>
                            {gold !== undefined && (
                              <td>
                                <Badge bg={gold === 'Deceptive' ? 'danger' : 'success'}>{gold}</Badge>
                              </td>
                            )}
                          </tr>
                        );
