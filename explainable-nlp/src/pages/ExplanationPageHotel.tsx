...
type HotelLabel = 'Deceptive' | 'Truthful';

function toHotelLabel(val: string | number | undefined): HotelLabel | undefined {
  if (val === undefined || val === null) return undefined;
  const t = String(val).trim().toUpperCase().replace(/[()\s.]/g, '');
  if (t === 'DECEPTIVE') return 'Deceptive';
  if (t === 'TRUTHFUL') return 'Truthful';
  return undefined;
}
...
const ExplanationPageHotel: React.FC = () => {
  const { datasetId, classificationId, resultId } = useParams();
  const navigate = useNavigate();

  const [entry, setEntry] = useState<HotelEntry | null>(null);
  ...
  const handlePrevious = () => {
    ...
    navigate(`/datasets/${datasetId}/classifications_hotel/${classificationId}/results/${newIndex}`);
  };
  const handleNext = () => {
    const newIndex = currentResultIndex + 1;
    navigate(`/datasets/${datasetId}/classifications_hotel/${classificationId}/results/${newIndex}`);
  ...
  if (loading) {
    return (
      <Container className="py-5 text-center">
        ...
        <div className="mt-3">Loading Hotel entry...</div>
      </Container>
    );
  }
  if (error) {
  ...
  const predHotelLabel = toHotelLabel(entry.label);
  const goldHotelLabel = toHotelLabel(entry.actualLabel);
    ...
    <Container className="py-4 explanation-page" fluid>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <Button
          ...
          onClick={() => navigate(entry?.method === 'explore' ? `/datasets/${datasetId}` : `/datasets/${datasetId}/classifications_hotel/${classificationId}`)}
        >
          ← Back to {entry?.method === 'explore' ? 'Datasets' : 'Hotel Classification'}
        </Button>
        ...
        <Col md={4}>
          <div className="d-flex flex-column gap-3">
            <div className="text-center">
              ...
              <Badge pill bg={predHotelLabel === 'Deceptive' ? 'danger' : predHotelLabel === 'Truthful' ? 'success' : 'secondary'} className="px-3 py-2 fs-6">
                {predHotelLabel || entry.label}
              </Badge>
            </div>
            <div className="text-center">
              <div className="text-muted small">Actual Label</div>
              <Badge pill bg={goldHotelLabel === 'Deceptive' ? 'danger' : goldHotelLabel === 'Truthful' ? 'success' : 'secondary'} className="px-3 py-2 fs-6">
                {goldHotelLabel || entry.actualLabel}
              </Badge>
            </div>
          </div>
        </Col>
...
export default ExplanationPageHotel;