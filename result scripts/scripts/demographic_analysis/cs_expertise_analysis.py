"""
CS/AI Expertise Analysis Script
================================

Analyzes the relationship between participants' CS/AI expertise (based on job field)
and key performance metrics using rigorous statistical methods.

Expertise Groups:
- Experts: Participants working in computer science, AI, software, tech fields
- Non-experts: All other participants
...
2. Regression with cluster-robust standard errors (main analysis, trial-level data)
   ...
   - Expertise coded as dummy variable (Non-expert as reference)

Visualizations:
- Box plots showing distribution by expertise
- Violin plots showing full distribution shapes
- AOR scatter plot (RAIR vs RSR) by expertise
...
# Keywords to identify CS/AI experts (from config)
CS_AI_KEYWORDS = config.CS_AI_KEYWORDS


def is_cs_ai_expert(job_text):
    """Check if job matches CS/AI keywords."""
    if pd.isna(job_text):
        return False
...
def add_expertise_to_long(long_df, df_wide, job_col):
    """
    Add participant-level CS/AI expertise classification to long format DataFrame.
    """
    # Create mapping from participant index to job field
    job_map = df_wide[job_col].to_dict()
    long_df['job_field'] = long_df['participant'].map(job_map)
    
    # Classify as expert/non-expert
...
def plot_boxplot_by_expertise(df, y_col, title, output_path):
    """
    Create box plot with expertise group on x-axis.
    """
    # Remove NaN values
    plot_data = df[['expertise_group', y_col]].dropna()
    ...
    # Order: Non-expert, Expert
    groups = ['Non-expert', 'CS/AI Expert']
    available_groups = [g for g in groups if g in plot_data['expertise_group'].unique()]
    positions = list(range(len(available_groups)))
    
    # Create box plot
    box_data = [plot_data[plot_data['expertise_group'] == g][y_col].values for g in available_groups]
    ...
    for i, group in enumerate(available_groups):
        group_data = plot_data[plot_data['expertise_group'] == group][y_col].values
...
def plot_metrics_by_expertise(participant_df, output_path):
    """
    Create separate subplots for each metric showing distributions by expertise.
    """
    ...
    for idx, (metric, label, color) in enumerate(zip(metrics, metric_labels, colors)):
        ax = axes[idx]
        plot_data = participant_df[['expertise_group', metric]].dropna()
        ...
        # Create violin plot
        violin_data = [plot_data[plot_data['expertise_group'] == g][metric].values for g in available_groups]
        ...
        for i, group in enumerate(available_groups):
            group_data = plot_data[plot_data['expertise_group'] == group][metric].values
            jitter = np.random.normal(0, 0.06, len(group_data))
            ax.scatter([i]*len(group_data) + jitter, group_data, alpha=0.3, s=25, color='black', edgecolors='none')
        
        ax.set_xlabel('Expertise Group', fontsize=11, fontweight='bold')
        ax.set_ylabel(label, fontsize=11, fontweight='bold')
        ax.set_title(label, fontsize=13, fontweight='bold')
        ax.set_xticks(positions)
        ax.set_xticklabels(available_groups, fontsize=9)
        ax.grid(True, alpha=0.2, linestyle='--', axis='y')
    
    # Remove the 6th subplot
    fig.delaxes(axes[5])
    
    plt.suptitle('Performance Metrics by CS/AI Expertise', fontsize=16, fontweight='bold', y=1.00)
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Saved: {output_path}")
...
def plot_aor_scatter_by_expertise(long_df, output_path):
    """
    Plot AOR scatter: RAIR (x-axis) vs RSR (y-axis) with one point per expertise group.
    ...
    rair_by_exp = df_rair.groupby("expertise_group")["rair"].mean().rename("RAIR")
    ...
    rsr_by_exp = df_rsr.groupby("expertise_group")["rsr"].mean().rename("RSR")
    
    # Combine
    summary = pd.concat([rair_by_exp, rsr_by_exp], axis=1).reset_index()
    summary = summary.dropna()  # Remove any group with missing data
    ...
    # Colors for expertise groups
    expertise_colors = {
        'CS/AI Expert': TUM_BLUE,
        'Non-expert': TUM_ORANGE
    }
    
    # Get colors for each group in summary
    point_colors = [expertise_colors.get(g, TUM_MED_BLUE) for g in summary['expertise_group']]
    
    # Scatter plot
    scatter = ax.scatter(summary["RAIR"], summary["RSR"], s=400, c=point_colors, 
                        edgecolor="black", linewidth=2.5, zorder=10, alpha=0.8)
    
    # Annotate each point with group and AOR
    for idx, row in summary.iterrows():
        label = f"{row['expertise_group']}\nAOR={row['AOR']:.3f}"
        ...
        ax.annotate(label, (row["RAIR"], row["RSR"]), 
                   ...
                   bbox=dict(boxstyle="round,pad=0.6", facecolor="white", 
                            edgecolor=expertise_colors.get(row['expertise_group'], TUM_MED_BLUE), 
    ...
    ax.set_title('Appropriateness of Reliance (AOR) by CS/AI Expertise\nRAIR vs RSR', 
    ...
    legend_elements = [Line2D([0], [0], marker='o', color='w', 
                             label=row['expertise_group'],
                             markerfacecolor=expertise_colors.get(row['expertise_group'], TUM_MED_BLUE), 
                             markeredgecolor='black', markersize=14)
                      for _, row in summary.iterrows()]
    ax.legend(handles=legend_elements, loc='lower right', title='Expertise', 
    ...
    # Print summary
    print("\n" + "="*80)
    print("AOR (Appropriateness of Reliance) by CS/AI Expertise")
    print("="*80)
    print(f"{'Expertise Group':<20} {'RAIR':<10} {'RSR':<10} {'AOR':<10}")
    print("-"*80)
    for _, row in summary.iterrows():
        print(f"{row['expertise_group']:<20} {row['RAIR']:<10.3f} {row['RSR']:<10.3f} {row['AOR']:<10.3f}")
    print("="*80)
    
    print(f"\nSaved AOR plot to: {output_path}")


def regression_analysis_clustered(long_df, expertise_var='is_expert'):
    """
    ...
    Expertise is coded as dummy variable (Non-expert = 0, Expert = 1).
    """
    results = {}
    ...
    print("Expertise coded as: Non-expert = 0 (reference), CS/AI Expert = 1")
    
    # 1. Plausibility (continuous) - OLS with cluster-robust SEs
    print("\n" + "-"*80)
    print("1. PLAUSIBILITY ~ CS/AI_Expertise (OLS with cluster-robust SEs)")
    print("-"*80)
    try:
        model_plaus = ols_clustered(f'plaus ~ {expertise_var}', data=analysis_df, cluster_var='participant')
        results['plausibility'] = model_plaus
        print(model_plaus.summary())
        coef = model_plaus.params[expertise_var]
        pval = model_plaus.pvalues[expertise_var]
        sig = '***' if pval < 0.001 else '**' if pval < 0.01 else '*' if pval < 0.05 else ''
        print(f"\nCoefficient: {coef:.4f} {sig}")
        print(f"Interpretation: CS/AI experts have {abs(coef):.4f} {'higher' if coef > 0 else 'lower'} plausibility ratings than non-experts")
    ...
    print("\n" + "-"*80)
    print("2. CONFIDENCE_CHANGE ~ CS/AI_Expertise (OLS with cluster-robust SEs)")
    print("-"*80)
    try:
        model_conf = ols_clustered(f'delta_conf ~ {expertise_var}', data=analysis_df, cluster_var='participant')
        results['confidence_change'] = model_conf
        print(model_conf.summary())
        coef = model_conf.params[expertise_var]
        pval = model_conf.pvalues[expertise_var]
        sig = '***' if pval < 0.001 else '**' if pval < 0.01 else '*' if pval < 0.05 else ''
        print(f"\nCoefficient: {coef:.4f} {sig}")
        print(f"Interpretation: CS/AI experts have {abs(coef):.4f} {'higher' if coef > 0 else 'lower'} confidence change than non-experts")
    except Exception as e:
        print(f"Error: {e}")
    ...
    print("\n" + "-"*80)
    print("3. FINAL_ACCURACY (post==gt) ~ CS/AI_Expertise (Logistic with cluster-robust SEs)")
    print("-"*80)
    try:
        analysis_df['final_correct'] = (analysis_df['post'] == analysis_df['gt']).astype(int)
        model_acc = logit_clustered(f'final_correct ~ {expertise_var}', data=analysis_df, cluster_var='participant')
        results['final_accuracy'] = model_acc
        print(model_acc.summary())
        coef = model_acc.params[expertise_var]
        pval = model_acc.pvalues[expertise_var]
        sig = '***' if pval < 0.001 else '**' if pval < 0.01 else '*' if pval < 0.05 else ''
        odds_ratio = np.exp(coef)
        print(f"\nLog-odds coefficient: {coef:.4f} {sig}")
        print(f"Odds ratio: {odds_ratio:.4f}")
        print(f"Interpretation: CS/AI experts have {odds_ratio:.4f}x the odds of being correct compared to non-experts")
    except Exception as e:
        print(f"Error: {e}")
    ...
    try:
        ...
        if len(rair_df) >= 10:
            model_rair = logit_clustered(f'changed_to_correct ~ {expertise_var}', data=rair_df, cluster_var='participant')
            results['rair'] = model_rair
            print(model_rair.summary())
            coef = model_rair.params[expertise_var]
            pval = model_rair.pvalues[expertise_var]
            sig = '***' if pval < 0.001 else '**' if pval < 0.01 else '*' if pval < 0.05 else ''
            odds_ratio = np.exp(coef)
            print(f"\nLog-odds coefficient: {coef:.4f} {sig}")
            print(f"Odds ratio: {odds_ratio:.4f}")
            print(f"Interpretation: CS/AI experts have {odds_ratio:.4f}x the odds of accepting AI's correct suggestion")
        else:
    ...
    try:
        ...
        if len(rsr_df) >= 10:
            model_rsr = logit_clustered(f'stayed_correct ~ {expertise_var}', data=rsr_df, cluster_var='participant')
            results['rsr'] = model_rsr
            print(model_rsr.summary())
            coef = model_rsr.params[expertise_var]
            pval = model_rsr.pvalues[expertise_var]
            sig = '***' if pval < 0.001 else '**' if pval < 0.01 else '*' if pval < 0.05 else ''
            odds_ratio = np.exp(coef)
            print(f"\nLog-odds coefficient: {coef:.4f} {sig}")
            print(f"Odds ratio: {odds_ratio:.4f}")
            print(f"Interpretation: CS/AI experts have {odds_ratio:.4f}x the odds of resisting AI's wrong suggestion")
        else:
...
def main():
    ...
    print("="*80)
    print("CS/AI EXPERTISE ANALYSIS")
    print("="*80)
    ...
    if JOB_COLUMN not in df_wide.columns:
        print(f"\nError: Column '{JOB_COLUMN}' not found in data.")
        print("Available columns containing 'job' or 'work':")
        relevant_cols = [c for c in df_wide.columns if 'job' in c.lower() or 'work' in c.lower() or 'field' in c.lower()]
    ...
    # Add expertise classification to long format
    long_df = add_expertise_to_long(long_df, df_wide, JOB_COLUMN)
    ...
    # Check expertise distribution
    expertise_dist = participant_metrics['expertise_group'].value_counts().sort_index()
    print("\nExpertise Distribution:")
    print(f"CS/AI Keywords used: {', '.join(CS_AI_KEYWORDS[:5])}... (and {len(CS_AI_KEYWORDS)-5} more)")
    for group, count in expertise_dist.items():
        if pd.notna(group):
    ...
    # Compute descriptive statistics by expertise
    print("\n" + "="*80)
    print("DESCRIPTIVE STATISTICS BY CS/AI EXPERTISE")
    print("="*80)
    
    desc_stats = {}
    for metric in metrics:
        desc_stats[metric] = participant_metrics.groupby('expertise_group')[metric].agg(['mean', 'std', 'count'])
        print(f"\n{metric}:")
    ...
    regression_results = regression_analysis_clustered(long_df, expertise_var='is_expert')
    ...
    for metric, title, filename in boxplot_configs:
        ...
        plot_boxplot_by_expertise(participant_metrics, metric, title, plot_path)
    
    # Multi-panel plot
    grouped_plot_path = os.path.join(OUTPUT_FOLDER, 'expertise_metrics_by_group.png')
    plot_metrics_by_expertise(participant_metrics, grouped_plot_path)
    
    # AOR scatter plot
    print("\nCreating AOR scatter plot...")
    aor_plot_path = os.path.join(OUTPUT_FOLDER, 'expertise_aor_scatter.png')
    plot_aor_scatter_by_expertise(long_df, aor_plot_path)
    ...
    print("  1. Descriptive statistics by expertise group")
    ...
    print("     - Same methodology as h_tests.py")
    print("     - Expertise as dummy variable (Non-expert = reference)")
    print("\nExpertise Classification:")
    print(f"  - CS/AI keywords: {', '.join(CS_AI_KEYWORDS[:8])}...")
    print("  - Based on job field/study")
    print("\nVisualization Types:")
    print("  - Box plots by expertise group")
    print("  - Multi-panel violin plots")
    print("  - AOR scatter plot by expertise")
    print("\nGenerated files:")
    print(f"  - {OUTPUT_FOLDER}/expertise_vs_rair.png (box plot)")
    print(f"  - {OUTPUT_FOLDER}/expertise_vs_rsr.png (box plot)")
    print(f"  - {OUTPUT_FOLDER}/expertise_vs_plausibility.png (box plot)")
    print(f"  - {OUTPUT_FOLDER}/expertise_vs_conf_delta.png (box plot)")
    print(f"  - {OUTPUT_FOLDER}/expertise_vs_accuracy.png (box plot)")
    print(f"  - {OUTPUT_FOLDER}/expertise_metrics_by_group.png (multi-panel violin plots)")
    print(f"  - {OUTPUT_FOLDER}/expertise_aor_scatter.png (AOR scatter: RAIR vs RSR)")
