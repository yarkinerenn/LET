...
def build_full_with_deltas_and_labels():
    ...
    # === JOB FILTER CONFIG ===
    FILTER_BY_JOB = config.FILTER_BY_JOB
    JOB_COLUMN = config.JOB_COLUMN
    FILTER_MODE = config.FILTER_MODE
    CS_AI_KEYWORDS = config.CS_AI_KEYWORDS

    # === Labels from config ===
    GT_LABELS = config.GT_LABELS
    AI_LABELS = config.AI_LABELS
    FAITH_LABELS = config.FAITH_LABELS
...
def main():
    ...
    df_with_initial_acc, initial_accuracy_global, initial_acc_summary = compute_initial_accuracy_per_user(df_trials, n_trials=config.N_TRIALS)
    
    # Compute final accuracy per user (using Q_ReviewExp)
    df_with_final_acc, final_accuracy_global, final_acc_summary = compute_final_accuracy_per_user(df_with_initial_acc, n_trials=config.N_TRIALS)

    # Compute global + per-user metrics in one go (and append columns)
    df_with_metrics, metrics_summary = compute_rair_rsr_global_and_per_user(df_with_final_acc, n_trials=config.N_TRIALS)
    ...
    if filter_by_job:
        if filter_mode == "include":
            output_excel_path = config.PROCESSED_DATA_FILE_CSAI_ONLY
        elif filter_mode == "exclude":
            output_excel_path = config.PROCESSED_DATA_FILE_NO_CSAI
        else:
            output_excel_path = config.DATA_DIR / "experiment_results_with_metrics_byjob.xlsx"
    else:
        output_excel_path = config.PROCESSED_DATA_FILE
    
    df_with_metrics.to_excel(str(output_excel_path), index=False)
